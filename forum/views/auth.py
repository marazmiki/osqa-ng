        ...
        if request.user.is_authenticated():
            if isinstance(assoc_key, (type, User)):
                ...
                try:
                    ...
                    uassoc.save()
                    messages.success(request, _('The new credentials are now associated with your account'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for u in users:
                ...
                messages.success(request, _("An email has been sent with your temporary login key"))
...
def send_validation_email(request):
    if not request.user.is_authenticated():
        ...
        messages.success(request, _("A message with an email validation link was just sent to your address."))
...
def auth_settings(request, id):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if is_new_pass:
                messages.success(request, _("New password set"))
...
def remove_external_provider(request, id):
    ...
    messages.success(request, _("You removed the association with %s") % association.provider)
    association.delete()
...
def login_and_forward(request, user, forward=None, message=None):
    ...
    if pending_data and (user.email_isvalid or pending_data['type'] not in settings.REQUIRE_EMAIL_VALIDATION_TO):
        ...
        if submission_time < datetime.datetime.now() - datetime.timedelta(minutes=int(settings.HOLD_PENDING_POSTS_MINUTES)):
            ...
            messages.success(request, (_("You have a %s pending submission.") % pending_data['data_name']) + " %s, %s, %s" % (
...
def forward_suspended_user(request, user, show_private_msg=True):
    ...
    request.user.message_set.create(message, level=messages_extends.SUCCESS_PERSISTENT)
    return HttpResponseRedirect(reverse('index'))
