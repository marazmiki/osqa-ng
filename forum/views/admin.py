        ...
        if form.is_valid():
            if 'preview' in request.POST:
                ...
                if not 'reset' in request.POST:
                    form.save()
                    messages.success(request, message=_("'%s' settings saved succesfully") % set_name)
...
#todo: this is the quick and dirty way of implementing a bootstrap mode
    ...
    messages.success(request, message=_('Bootstrap mode enabled'))
    return HttpResponseRedirect(reverse('admin_index'))
...
def go_defaults(request):
    ...
    messages.success(request, message=_('All values reverted to defaults'))
    return HttpResponseRedirect(reverse('admin_index'))
...
def recalculate_denormalized(request):
    ...
    messages.success(request, message=_('All values recalculated'))
    return HttpResponseRedirect(reverse('admin_index'))
...
def maintenance(request):
    if request.POST:
        if 'close' in request.POST or 'adjust' in request.POST:
            ...
            messages.success(request, message=_("Your site is now running normally"))
            return HttpResponseRedirect(reverse('admin_maintenance'))
    else:
...
def create_user(request):
    if request.POST:
        ...
        if form.is_valid():
            ...
            messages.success(request, message=_("New user created sucessfully. %s.") % html.hyperlink(
