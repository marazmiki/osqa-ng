...
class User(BaseModel, DjangoUser):
    ...
    def get_vote_count_today(self):
        today = datetime.date.today() ### @TODO ?
    ...
    @true_if_is_super_or_staff
    def can_edit_comment(self, comment):
        return (comment.author == self and comment.added_at >= now() - datetime.timedelta(minutes=60)
    ...
    def is_suspended(self):
        if not self.is_active:
            ...
            now() > suspension.action_date + datetime.timedelta(
