    ...
    def repute_users(self):
        ...
        if self._value > 0:
            self._affected.message_set.create(
                    ...
                    '<br />%s' % self.extra.get('message', _('Thank you')), level=messages_extends.SUCCESS_PERSISTENT)
        else:
            self._affected.message_set.create(
                    message=_("You have been penalized in %s reputation points.") % self._value +
                    '<br />%s' % self.extra.get('message', ''), level=messages_extends.SUCCESS_PERSISTENT)
...
class AwardAction(ActionProxy):
    ...
    def process_action(self):
        ...
        self.user.message_set.create(message=_(
            ...
            badge_url=award.badge.get_absolute_url()), level=messages_extends.SUCCESS_PERSISTENT)
